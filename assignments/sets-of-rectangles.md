...
Updates
========
- 05/13/2026: The BoxSet interface contains a method called `size()`, but the documentation is missing. This method must return the number of boxes in the `BoxSet`.
- 05/13/2026: Tests for `SimpleBoxSet` must be in `src/test/java/box/SimpleBoxSetTest.java`. If VSCode autogenerated an empty test file for you, that is where it places the file. If you your tests for `SimpleBoxSet` are elsewhere, make the above file and put them there.
