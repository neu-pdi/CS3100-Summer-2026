...
**Format:** Multiple Choice (17 questions)
**Version:** Practice
...
### Question 3
In CookYourBooks, `ExactQuantity`, `FractionalQuantity`, and `RangeQuantity` all extend `Quantity`. Which of the following must hold true for this relationship to be as intended?
...
### Question 4
In the `IOTDevice` example, the `BaseIOTDevice` class was declared as abstract rather than concrete. Why?

- a) Abstract classes use less memory
- b) It has abstract methods like `identify()` that subclasses must implement, and it should not be possible to instantiate `BaseIOTDevice` directly
...
devices.add(new Light("bedroom light",100)); //a light with power of 100
devices.add(new Fan("living room fan",3)); // a fan with speed of 3
```

- a) Nothing; this is a fine practice
- b) Without generics, the compiler cannot prevent adding wrong types to the list
...
### Question 7
A `Person` class is written that includes the first name, last name and year of birth of a single person. In `Person.hashCode()`, the first and last names are converted to lowercase before hashing. Which of the following are plausible reasons for this?
...
- a) A good specification should rule out bad implementation
- b) A good specification should not over-constrain the implementation
- c) A good specification should include performance requirements
- d) A good specification should be easy to understand
...
### Question 10
A class `MeasuredIngredient` overrides both `equals()` and `hashCode()`. Why is it important to override both together?
...
- b) `BufferedInputStream` can be used to wrap any `InputStream` at runtime without creating a separate subclass for every combination
...
| 17 | B | L8: Composition over inheritance |