...
- **Prerequisites:** Students understand coupling/cohesion, SOLID principles, test doubles
...
- These concepts build directly on information hiding and coupling/cohesion
- Testability is a *consequence* of following SOLID principles
...
- Information hiding can reduce observability if overdone
- Balance is key
...
<div style={{fontSize: '0.7em', marginTop: '0.5em'}}>
  ...
  <p className="fragment" style={{color: '#2196F3', margin: '0.25em 0'}}>💡 This is Dependency Inversion Principle in action!</p>
</div>
...
<p style={{fontSize: '0.85em'}}>
  Testability isn't a separate concern—it's a natural consequence of following the principles from earlier design lectures:
</p>
...
|------------------|---------------------|
| **Low Coupling ** | Dependencies can be substituted with test doubles |
| **High Cohesion ** | Each class has a clear purpose → focused tests |
| **Information Hiding ** | Implementation changes don't break tests |
| **Dependency Inversion ** | Inject test doubles instead of real dependencies |
| **Single Responsibility ** | Fewer behaviors to test per class |
| **Interface Segregation ** | Smaller interfaces → simpler test doubles |
...
<p style={{fontSize: '0.75em', marginTop: '0.5em', color: '#9370DB'}}>
  Connection to Single Responsibility Principle: DeviceHealthChecker was doing two things. Health assessment deserves its own class.
</p>
...
<aside className="notes">
**Connection to prior lectures:**
- Information hiding → isolate infrastructure behind interfaces
- Coupling/cohesion → low coupling enables test double substitution
- SOLID → Dependency Inversion is key to testability
