...
Previously we learned to hide implementation details behind interfaces.
</p>
...
1. **Hidden dependencies**: Not visible in constructor — discovery requires reading every line (common coupling!)
2. **Untestable**: Can't substitute a test double (this is why we inject dependencies!)
...
- Singleton creates common coupling (shared global state)
- Singleton hurts testability (can't substitute test doubles)
- We now see the full creation pattern perspective
...
## Dependency Injection Solves All Three Singleton Problems 
...
| **Testability** | Can't substitute | Inject stubs/mocks freely |
...
<p style={{fontSize: '0.85em'}}>
<em>"Software Engineering is the Integral of Programming Over Time"</em>
</p>
...
<p style={{fontSize: '0.85em'}}>
Dependency Injection and Service Locator make <strong>different coupling tradeoffs</strong>:
</p>
...
<ul style={{fontSize: '0.72em'}}>
  ...
  <li><strong>Singleton is an anti-pattern:</strong> hides dependencies, hurts testability, creates global state</li>
  <li><strong>Dependency Injection</strong> is both a testability enabler AND a creation pattern — same technique!</li>
...
- Information hiding → creation patterns hide construction
- Coupling → Singleton creates problematic coupling
- DIP → DI applies dependency inversion
- Testability → DI enables test double substitution
