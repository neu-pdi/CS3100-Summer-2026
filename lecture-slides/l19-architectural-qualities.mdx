...
- **Prerequisites:** (testability, hexagonal intro), (creation patterns, DI), (architectural thinking, boundaries, C4, ADRs, Pawtograder/Bottlenose, **four heuristics**)
- **Connects to:** (distributed architecture, networks, client-server, serverless), (Conway's Law, teams), (MVC/GUI)
...
<strong>Conway's Law</strong>: Organizations design systems that mirror their communication structure. If you have a "frontend team" and "backend team," you'll get technical partitioning. If you have a "grading team" and "courses team," you'll get domain partitioning.
</p>
</div>
...
- The deep exploration of how architectural choices affect these attributes comes LATER in this lecture (when we examine styles)
...
These three attributes become critical when comparing monoliths vs. distributed systems. We'll explore them in depth later — for now, just the vocabulary:
</p>
...
Notice the pattern: Monoliths tend to have <strong>better responsiveness</strong> but <strong>worse deployability and fault tolerance</strong>. Distributed systems flip this. 
</p>
</div>
...
- Distributed architecture lecture will unpack this in detail with real examples
...
This is the "big picture" attribute — we'll see how styles affect it throughout this lecture and Distributed Architecture lecture.
</p>
...
Scaling strategies vary by architecture — we'll explore this in depth in Distributed Architecture lecture.
</p>
...
<div className="fragment">
<p style={{fontSize: '0.78em', marginTop: '0.75em', fontWeight: 'bold', color: '#9370DB'}}>
Key insight: Scalability isn't just "can the system handle more load?" It's "does the <em>rest</em> of the system stay responsive while handling that load?" We'll return to this distinction later.
</p>
</div>
...
*This is where industry has been moving.*
...
**Preview for Distributed Architecture lecture:**
...
|-------------------|---------------|
| **"The Network Changes Everything"** | Fallacies of Distributed Computing, client-server architecture, security across trust boundaries |
| **Monolith vs. Microservices** | Distributed architecture styles, when to break the monolith, serverless |
| **Quality Attribute Tradeoffs** | How platform constraints (serverless, containers) shape architecture — like GitHub Actions shaped Pawtograder |
| **Heuristics → Emergent Styles** | Conway's Law — team structure is another heuristic that shapes architecture |
...
<aside className="notes">
**The arc ahead:**
- **(Networks & Security):** When components communicate over a network, everything gets harder. Fallacies of Distributed Computing. Client-server architecture. Security.
- **(Serverless):** Infrastructure building blocks — databases, blob storage, queues. Serverless as an architectural style.
- **(Teams):** Conway's Law — team structure is ANOTHER heuristic. "Organizations design systems that mirror their communication structure."

**The throughline with heuristics:**
- "How do we design for testability?" → Hexagonal Architecture emerges
- "Where do the boundaries go?" → Four heuristics: Rate of Change, Actor, ISP, Testability
- "What styles emerge from those heuristics?" → Hexagonal, Layered, Pipelined, Monolith
- "What happens when boundaries cross networks?" → Distributed styles emerge
- "What happens when boundaries cross teams?" → Conway's Law as another heuristic
