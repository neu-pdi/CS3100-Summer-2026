...
- **Prerequisites:** (monoliths, modular monoliths, microservices intro, Conway's Law preview)
- **Connects to:** (serverless — pushing these concerns to the platform), (teams and Conway's Law)
...
<aside className="notes">
**Context:**
- Previous lecture ended with a tease: the road continues into fog, "Distributed Systems Ahead, Here Be Dragons"
- Today we go into that fog
- Running examples: Pawtograder and Bottlenose — but now we focus on the network boundary between them

**Framing the lecture:**
- "Previous lecture was about how we organize code INSIDE a single deployment"
...
- This maps to Conway's Law: team structure → system structure
...
<p style={{fontSize: '0.82em'}}>
Previously we introduced Conway's Law:
</p>
...
We'll explore this further in (Teams and Collaboration).
</p>

<aside className="notes">
**Connect the whole arc:**
- "Where do boundaries go?" → the four heuristics
- "How do we organize code?" → architectural styles
- "What changes when we cross the network?" → distributed system challenges
- "How do teams shape architecture?" → Conway's Law in depth
