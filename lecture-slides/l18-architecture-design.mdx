...
- **Prerequisites:** (creation patterns, DI, service wiring), (testability), (information hiding, coupling)
- **Connects to:** (Architectural Qualities), (Networks & Security), (Sustainability)
...
<aside className="notes">
**Context:**
- Previous ended with creation patterns and DI — how do we wire services together?
...
- We'll cover networks, authentication, deployment
...
**Connection to (Service Locator vs DI):**
...
<p style={{fontSize: '0.85em'}}>
Once we've identified components, we need to design their interfaces. The same principles from class design apply.
</p>
...
<aside className="notes">
**Connection to creation patterns:**
- We passed a `ConversionRegistry` to `Recipe.convert()`. At the service level, the same principle applies—but now we're injecting entire collaborating services.
