...
In this lab you will pair up with a partner and build a working GUI
together. You will use the SceneItAll domain. The starter code provides the Model classes (`Light`, `Fan`,
`Shade`, `Area`, `Scene`), a ViewModel skeleton, and test scaffolding. You will fill in the View, Controller, and tests — together.
...
Find a partner. Sit at one computer together. Decide who drives
first (you will swap in Part 2).

## 2.2 Get the starter project

**One person** will clone this lab's repository from Pawtograder. This is the shared codebase you will both work on.
...
NullAway and `@FXML` fields `@FXML` fields are injected by `FXMLLoader` at runtime, after the constructor runs — so they look uninitialized to NullAway. The skeleton Controllers handle this with `@NullUnmarked` (from JSpecify) at the top of the class, which tells NullAway that null analysis
...
# 3 Part 2: Build Your Area Dashboard — Pair Programming
...
## 3.1 Phase A: Build the FXML (Driver 1)

**Driver** builds the layout in Scene Builder. **Navigator** guides the layout decisions.
...
**Navigator:** Check that every `fx:id` follows a consistent naming convention. Check that every interactive widget has `accessibleText`.

**🔄 Swap roles!**

## 3.2 Phase B: Wire ViewModel & Controller (Driver 2)
...
Implement ViewModel methods before wiring Controller listeners If your Controller sets up
...
**Navigator:** Watch for the #1 lifecycle bug — code that touches `@FXML` fields in the constructor instead of `initialize()`. Watch for `fx:id` / field name mismatches. Your task may require widgets not shown in the reference (e.g., `TextField`, `ComboBox<Enum>`) — check the JavaFX docs or ask your partner. Think ahead: "after we wire this, how will we test it?"

**🔄 Swap roles!**

## 3.3 Phase C: Run & Polish (Both)
...
## 4.1 Write one ViewModel test (one partner drives)
...
## 5.2 Section 2: Pair Programming
